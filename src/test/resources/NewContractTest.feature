Feature: NewContract
  ...
  Scenario: If the data of the contract is valid
    ...
    When the organizer request to "/organizers/2/musicians/8/districts/1/contracts" to contract for an available time, date and location with the following data is valid
    Then the response status code for this new contract request is 200

  Scenario: If the data of the contract is not valid
    Given that the organizer want to send a contract to the musician
    When the organizer request to "/organizers/2/musicians/8/districts/1/contracts" to contract for an available time, date and location with the following data is not valid
